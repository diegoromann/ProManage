...
['Abrir Dashboard','dash'],
['Abrir Reportes','reports'],
['Abrir Pomodoro','__pomodoro__']
];
...
if(typeof switchSection === 'function'){
  if(a[1] === '__pomodoro__') {
    switchSection('dash');
    setTimeout(()=>{
      const target = document.getElementById('dashPomodoroDisplay') || document.querySelector('[id*=\"Pomodoro\"], [id*=\"pomodoro\"], .pomodoro');
...
/* arranque estable: sin polling ni listeners globales en captura para evitar congelamientos */
const oldSwitch=window.switchSection;
...
setTimeout(injectSystemHealth,900); document.addEventListener('click',()=>setTimeout(injectSystemHealth,300),{passive:true});
})();

/* Old responsive patch removed: replaced by final stable drawer/theme patch. */

/* =========================================================
   PROMANAGE FINAL STABILITY PATCH - menu móvil, perfiles, claro, PDF y ficha 360
   Sin loops, sin polling agresivo, sin overlays bloqueando el sidebar.
   ========================================================= */
(function(){
  'use strict';
  ...
  const money = v => new Intl.NumberFormat('es-GT',{style:'currency',currency:'GTQ',maximumFractionDigits:2}).format(Number(v||0));
  function readJSON(key, fallback){ try{ const raw=localStorage.getItem(key); return raw ? JSON.parse(raw) : fallback; }catch(e){ return fallback; } }
  function getCurrentUser(){ try{ if(typeof currentUser !== 'undefined' && currentUser) return currentUser; }catch(e){} return readJSON('pm_currentUser', null); }
  function getSelectedCompany(){ try{ if(typeof selectedCompanyId !== 'undefined' && selectedCompanyId) return selectedCompanyId; }catch(e){} return localStorage.getItem('pm_selectedCompanyId') || ''; }
  function isAdmin(){ const u=getCurrentUser(); return !!u && (u.role === 'admin' || low(u.email) === 'admin@promanage.com' || u.id === 'admin'); }
  function rows(key, liveName){ try{ if(liveName && typeof window[liveName] !== 'undefined' && Array.isArray(window[liveName])) return window[liveName]; }catch(e){} try{ if(liveName && eval('typeof '+liveName) !== 'undefined' && Array.isArray(eval(liveName))) return eval(liveName); }catch(e){} return readJSON(key, []); }
  function scoped(list){ const cid=getSelectedCompany(); if(isAdmin()) return Array.isArray(list) ? list : []; return (Array.isArray(list) ? list : []).filter(x => !cid || !x || !x.companyId || String(x.companyId) === String(cid)); }
  function projectsScoped(){ return scoped(rows('pm_projects','projects')); }
  function financesScoped(){ return scoped(rows('pm_finances','finances')); }
  function inventoryScoped(){ return scoped(rows('pm_inventory','inventory')); }
  function logsScoped(){ return scoped(rows('pm_activityLogs','activityLogs').concat(readJSON('pm_enterpriseTimeline',[]))); }

  function dashboardVisible(){ const d=$('dashboardWrapper'); if(!d) return false; return getComputedStyle(d).display !== 'none'; }
  function ensureMobileDrawer(){
    ...
    if(!btn){
      ...
      btn.innerHTML='<span class="pm-menu-icon" aria-hidden="true"><i></i><i></i><i></i></span><strong>Menú</strong>';
      document.body.appendChild(btn);
    }
    let bd=$('pmMobileSidebarBackdrop');
    if(!bd){ bd=document.createElement('div'); bd.id='pmMobileSidebarBackdrop'; bd.className='pm-mobile-sidebar-backdrop'; document.body.appendChild(bd); }
  }
  function syncShell(){
    ensureMobileDrawer();
    document.body.classList.toggle('pm-dashboard-active', dashboardVisible());
    document.body.classList.toggle('pm-mobile-layout', dashboardVisible() && MOBILE.matches);
    syncTheme();
    if(!dashboardVisible() || !MOBILE.matches) closeSidebarFinal();
  }
  function openSidebarFinal(){
    ensureMobileDrawer();
    if(!dashboardVisible()) return;
    const s=$('sidebar'), bd=$('pmMobileSidebarBackdrop'), btn=$('pmMobileSidebarToggle');
    if(s){ s.classList.add('active','pm-sidebar-visible'); s.style.pointerEvents='auto'; }
    if(bd && MOBILE.matches){ bd.classList.add('active'); }
    if(btn) btn.setAttribute('aria-expanded','true');
    document.body.classList.add('pm-sidebar-open');
  }
  function closeSidebarFinal(){
    const s=$('sidebar'), bd=$('pmMobileSidebarBackdrop'), btn=$('pmMobileSidebarToggle');
    if(s){ s.classList.remove('active','pm-sidebar-visible'); }
    if(bd) bd.classList.remove('active');
    if(btn) btn.setAttribute('aria-expanded','false');
    document.body.classList.remove('pm-sidebar-open');
  }
  window.toggleSidebar = function(force){ if(force === true) return openSidebarFinal(); if(force === false) return closeSidebarFinal(); const s=$('sidebar'); return s && (s.classList.contains('active') || s.classList.contains('pm-sidebar-visible')) ? closeSidebarFinal() : openSidebarFinal(); };
  window.openMobileSidebar = openSidebarFinal;
  window.closeMobileSidebar = closeSidebarFinal;

  function syncTheme(){
    const html=document.documentElement;
    const saved=localStorage.getItem('theme') || localStorage.getItem('pm_theme') || html.getAttribute('data-theme') || 'dark';
    const theme = saved === 'light' ? 'light' : 'dark';
    html.setAttribute('data-theme', theme);
    document.body.setAttribute('data-theme', theme);
    const meta=document.querySelector('meta[name="theme-color"]');
    if(meta) meta.setAttribute('content', theme === 'light' ? '#f4f8ff' : '#020713');
  }
  const oldToggleTheme = window.toggleTheme;
  window.toggleTheme = function(){
    let result;
    if(typeof oldToggleTheme === 'function') result = oldToggleTheme.apply(this, arguments);
    else {
      const now = (document.documentElement.getAttribute('data-theme') === 'light') ? 'dark' : 'light';
      localStorage.setItem('theme', now); localStorage.setItem('pm_theme', now); document.documentElement.setAttribute('data-theme', now);
    }
    setTimeout(syncTheme,0); setTimeout(syncTheme,80);
    return result;
  };

  function refreshReportProjectsFinal(){
    const select=$('reportProjectSelect'); if(!select) return;
    const current=select.value;
    const list=projectsScoped();
    select.innerHTML='';
    if(!list.length){ select.innerHTML='<option value="">No hay proyectos disponibles</option>'; return; }
    list.forEach(p=>{ const opt=document.createElement('option'); opt.value=norm(p.id); opt.dataset.projectId=norm(p.id); opt.textContent=p.title || p.name || 'Proyecto sin nombre'; select.appendChild(opt); });
    if([...select.options].some(o=>o.value===current)) select.value=current;
  }
  window.pmRefreshReportProjects = refreshReportProjectsFinal;
  window.renderReportProjects = refreshReportProjectsFinal;

  function projectImages(project){
    const id=norm(project && project.id); const out=[]; const seen=new Set();
    function add(x,i){
      if(!x) return; const src=x.dataUrl || x.url || x.src || x.base64 || x.image || x.content || '';
      if(!String(src).startsWith('data:image')) return; if(seen.has(src)) return; seen.add(src);
      out.push({src,name:x.name||x.filename||('Imagen '+(i+1)),createdAt:x.createdAt||x.date||x.uploadedAt||'',size:x.size||0});
    }
    function addList(a){ if(Array.isArray(a)) a.forEach(add); }
    try{ if(typeof window.getProjectImages === 'function') addList(window.getProjectImages(id)); }catch(e){}
    ['images','gallery','media','attachments','files','evidences','evidence'].forEach(k=>addList(project && project[k]));
    ['pm_projectImages','pm_project_images','pm_projectsImages','pm_gallery','pm_media','pm_attachments'].forEach(key=>{
      const data=readJSON(key,null); if(!data) return;
      if(Array.isArray(data)) addList(data.filter(x=>norm(x.projectId||x.project||x.parentId)===id));
      else if(Array.isArray(data[id])) addList(data[id]);
    });
    return out;
  }
  function findProject(id){ const val=norm(id); return projectsScoped().find(p=>norm(p.id)===val) || null; }
  function relatedFinances(p){ const id=low(p.id), title=low(p.title||p.name); return financesScoped().filter(f=>{ const s=low([f.projectId,f.project,f.parentId,f.category,f.concept,f.description,f.name].join(' ')); return s.includes(id) || (title && s.includes(title)); }); }
  function relatedInventory(p){ const id=low(p.id), title=low(p.title||p.name); return inventoryScoped().filter(i=>{ const s=low([i.projectId,i.project,i.parentId,i.product,i.name,i.category,i.description].join(' ')); return s.includes(id) || (title && s.includes(title)); }); }
  function relatedLogs(p){ const id=low(p.id), title=low(p.title||p.name); return logsScoped().filter(l=>{ const s=low([l.projectId,l.project,l.parentId,l.message,l.text,l.title,l.description,l.m].join(' ')); return s.includes(id) || (title && s.includes(title)); }).slice(0,12); }

  function ensure360Modal(){
    let modal=$('pmProject360Clean');
    if(!modal){
      modal=document.createElement('div'); modal.id='pmProject360Clean'; modal.className='pm360-clean-modal';
      modal.innerHTML='<div class="pm360-clean-card"><button class="pm360-clean-close" type="button" aria-label="Cerrar">×</button><div id="pm360CleanContent"></div></div>';
      document.body.appendChild(modal);
      modal.addEventListener('click', e=>{ if(e.target===modal || e.target.closest('.pm360-clean-close')) modal.classList.remove('active'); });
    }
    return modal;
  }
  window.openProject360 = function(projectId){
    const p=findProject(projectId); if(!p){ alert('Proyecto no encontrado.'); return; }
    const imgs=projectImages(p), fins=relatedFinances(p), inv=relatedInventory(p), logs=relatedLogs(p);
    const ingresos=fins.filter(f=>low(f.type).includes('ingreso')).reduce((a,f)=>a+Number(f.amount||f.value||f.net||0),0);
    const egresos=fins.filter(f=>!low(f.type).includes('ingreso')).reduce((a,f)=>a+Number(f.amount||f.value||f.net||0),0);
    const gallery=imgs.length ? imgs.map(i=>`<figure><img src="${i.src}" alt="${esc(i.name)}"><figcaption>${esc(i.name)}</figcaption></figure>`).join('') : '<div class="pm360-empty">Este proyecto no tiene imágenes vinculadas todavía.</div>';
    const finRows=fins.length ? fins.map(f=>`<div class="pm360-line"><span>${esc(f.category||f.concept||f.name||'Movimiento')}</span><strong>${esc(f.type||'movimiento')} · ${money(f.amount||f.value||f.net||0)}</strong></div>`).join('') : '<div class="pm360-empty">No hay finanzas relacionadas solamente a este proyecto.</div>';
    const invRows=inv.length ? inv.map(i=>`<div class="pm360-line"><span>${esc(i.product||i.name||'Insumo')}</span><strong>${esc(i.stock||i.qty||i.quantity||0)} uds.</strong></div>`).join('') : '<div class="pm360-empty">No hay inventario vinculado directamente.</div>';
    const logRows=logs.length ? logs.map(l=>`<div class="pm360-line"><span>${esc(l.message||l.text||l.title||l.m||'Actividad')}</span><small>${esc(l.date||l.createdAt||l.t||'')}</small></div>`).join('') : '<div class="pm360-empty">No hay actividad específica registrada.</div>';
    const modal=ensure360Modal();
    $('pm360CleanContent').innerHTML=`<header class="pm360-head"><p>Ficha completa del proyecto</p><h2>${esc(p.title||p.name||'Proyecto')}</h2><span>${esc(p.description||p.desc||'Sin descripción registrada.')}</span><div class="pm360-actions"><button class="btn-primary" type="button" onclick="document.getElementById('reportProjectSelect')&&(document.getElementById('reportProjectSelect').value='${esc(p.id)}'); window.generateSingleProjectPDF&&window.generateSingleProjectPDF();">Generar PDF</button><button class="btn-secondary" type="button" onclick="document.getElementById('pmProject360Clean').classList.remove('active')">Cerrar</button></div></header><section class="pm360-kpis"><div><span>Estado</span><strong>${esc(p.status||'No definido')}</strong></div><div><span>Prioridad</span><strong>${esc(p.priority||'No definida')}</strong></div><div><span>Imágenes</span><strong>${imgs.length}</strong></div><div><span>Balance</span><strong>${money(ingresos-egresos)}</strong></div></section><section class="pm360-layout"><article><h3>Galería vinculada</h3><div class="pm360-gallery">${gallery}</div></article><article><h3>Finanzas del proyecto</h3>${finRows}</article><article><h3>Inventario relacionado</h3>${invRows}</article><article><h3>Actividad</h3>${logRows}</article></section>`;
    modal.classList.add('active');
  };
  function projectIdFromOnclick(el){ const s=norm(el && el.getAttribute && el.getAttribute('onclick')); const m=s.match(/'([^']+)'/); return m ? m[1] : ''; }
  function inject360Buttons(){
    document.querySelectorAll('#projectsListTableBody tr').forEach(row=>{
      if(row.querySelector('.pm360-btn')) return;
      const source=row.querySelector('[onclick*="Project"]'); const id=projectIdFromOnclick(source); if(!id) return;
      const td=row.querySelector('td:last-child') || row.lastElementChild; if(!td) return;
      const b=document.createElement('button'); b.type='button'; b.className='action-text-btn pm360-btn'; b.textContent='Ficha 360'; b.onclick=e=>{ e.preventDefault(); e.stopPropagation(); openProject360(id); };
      td.insertBefore(b, td.firstChild);
    });
    document.querySelectorAll('.project-card').forEach(card=>{
      if(card.querySelector('.pm360-btn')) return;
      const source=card.querySelector('[onclick*="Project"]'); const id=projectIdFromOnclick(source); if(!id) return;
      const b=document.createElement('button'); b.type='button'; b.className='action-text-btn pm360-btn'; b.textContent='Ficha 360'; b.onclick=e=>{ e.preventDefault(); e.stopPropagation(); openProject360(id); };
      (card.querySelector('.project-info') || card).appendChild(b);
    });
  }
  const oldSwitch=window.switchSection;
  if(typeof oldSwitch === 'function') window.switchSection=function(section){
    const fixed = section === 'dashboard' ? 'dash' : (section === 'pomodoro' ? 'dash' : section);
    const r=oldSwitch.call(this,fixed);
    setTimeout(()=>{ syncShell(); refreshReportProjectsFinal(); inject360Buttons(); if(section === 'pomodoro'){ const t=$('dashPomodoroDisplay') || $('btnDashPomodoroToggle'); if(t) t.scrollIntoView({behavior:'smooth',block:'center'}); } },120);
    return r;
  };
  const oldSelect=window.selectCompany;
  if(typeof oldSelect === 'function') window.selectCompany=function(){ const r=oldSelect.apply(this,arguments); setTimeout(()=>{ syncShell(); refreshReportProjectsFinal(); inject360Buttons(); },140); return r; };

  document.addEventListener('click', function(e){
    const t=e.target;
    if(t && t.closest && t.closest('#pmMobileSidebarToggle')){ e.preventDefault(); e.stopPropagation(); window.toggleSidebar(); return; }
    if(t && t.closest && t.closest('#pmMobileSidebarBackdrop')){ e.preventDefault(); closeSidebarFinal(); return; }
    if(MOBILE.matches && t && t.closest && t.closest('#sidebar .menu-item')) setTimeout(closeSidebarFinal,120);
    if(t && t.closest && t.closest('.icon-search-btn')){ const shell=$('pageSearchShell'); if(shell){ shell.classList.add('expanded','active'); setTimeout(()=>$('globalSearchInput')?.focus(),80); } }
  }, false);
  document.addEventListener('keydown', function(e){
    if((e.ctrlKey || e.metaKey) && low(e.key)==='k') setTimeout(()=>{
      document.querySelectorAll('#paletteResults div').forEach(item=>{
        const txt=low(item.textContent);
        if(txt.includes('dashboard')) item.onclick=()=>{ window.switchSection&&window.switchSection('dash'); document.getElementById('pmPalette')?.remove(); };
        if(txt.includes('pomodoro')) item.onclick=()=>{ window.switchSection&&window.switchSection('pomodoro'); document.getElementById('pmPalette')?.remove(); };
      });
    },30);
  }, true);

  function init(){
    try{ ensureMobileDrawer(); syncTheme(); syncShell(); refreshReportProjectsFinal(); ensure360Modal(); inject360Buttons(); }catch(e){ console.warn('ProManage final patch omitido:', e); }
    setTimeout(()=>{ syncShell(); refreshReportProjectsFinal(); inject360Buttons(); },250);
    setTimeout(()=>{ const splash=$('pmSplash'), loader=$('pmLoader'); [splash,loader].forEach(el=>{ if(el){ el.classList.add('hidden'); el.style.pointerEvents='none'; el.style.display='none'; }}); },1400);
  }
  window.addEventListener('resize', ()=>setTimeout(syncShell,80), {passive:true});
  window.addEventListener('orientationchange', ()=>setTimeout(syncShell,180), {passive:true});
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init, {once:true}); else init();
})();

/* =========================================================
   PROMANAGE HARD FIX 2026-05-31
   Capa final sin loops: drawer móvil clickable + sidebar claro real.
   ========================================================= */
(function(){
  'use strict';
  ...
  function isDashboard(){ return visible($('dashboardWrapper')); }
  function ensureControls(){
    ...
    return {btn:btn, bd:bd};
  }
  function placeSidebar(){
    var s=$('sidebar'), dash=$('dashboardWrapper');
    if(!s || !dash) return;
    if(mq.matches && isDashboard()){
      if(s.parentNode !== document.body){
        sidebarPlaceholder = sidebarPlaceholder || document.createComment('promanage-sidebar-placeholder');
        if(!sidebarPlaceholder.parentNode) s.parentNode.insertBefore(sidebarPlaceholder, s);
        document.body.appendChild(s);
      }
    }else if(sidebarPlaceholder && sidebarPlaceholder.parentNode && s.parentNode === document.body){
      sidebarPlaceholder.parentNode.insertBefore(s, sidebarPlaceholder);
      sidebarPlaceholder.parentNode.removeChild(sidebarPlaceholder);
    }
  }
  function syncThemeHard(){
    var html=document.documentElement;
    var theme=localStorage.getItem('pm_theme') || localStorage.getItem('theme') || html.getAttribute('data-theme') || document.body.getAttribute('data-theme') || 'dark';
    theme = theme === 'light' ? 'light' : 'dark';
    html.setAttribute('data-theme', theme);
    document.body.setAttribute('data-theme', theme);
    localStorage.setItem('pm_theme', theme);
    localStorage.setItem('theme', theme);
    var txt=$('themeToggleText');
    if(txt) txt.textContent = theme === 'light' ? 'Modo Oscuro' : 'Modo Claro';
  ...
  function syncLayout(){
    ensureControls();
    syncThemeHard();
    document.body.classList.toggle('pm-dashboard-active', isDashboard());
    document.body.classList.toggle('pm-mobile-layout', isDashboard() && mq.matches);
    placeSidebar();
    if(!mq.matches || !isDashboard()) closeSidebarHard();
  }
  function openSidebarHard(){
    ensureControls();
    if(!isDashboard()) return;
    placeSidebar();
    var s=$('sidebar'), bd=$('pmMobileSidebarBackdrop'), btn=$('pmMobileSidebarToggle');
    if(s){
      s.classList.add('active','pm-sidebar-visible');
      s.style.zIndex='10000';
      s.style.pointerEvents='auto';
      if(mq.matches) s.style.transform='translate3d(0,0,0)';
    }
    if(bd && mq.matches){
    ...
    if(btn){ btn.setAttribute('aria-expanded','true'); btn.style.zIndex='11000'; }
    document.body.classList.add('pm-sidebar-open');
  }
  function closeSidebarHard(){
    var s=$('sidebar'), bd=$('pmMobileSidebarBackdrop'), btn=$('pmMobileSidebarToggle');
    if(s){
      s.classList.remove('active','pm-sidebar-visible');
      if(mq.matches) s.style.transform='translate3d(-112%,0,0)';
  ...
  window.toggleSidebar = function(force){
    var s=$('sidebar');
    if(force === true) return openSidebarHard();
    if(force === false) return closeSidebarHard();
    return s && (s.classList.contains('active') || s.classList.contains('pm-sidebar-visible')) ? closeSidebarHard() : openSidebarHard();
  };
  window.openMobileSidebar = openSidebarHard;
  window.closeMobileSidebar = closeSidebarHard;

  var previousToggle = window.toggleTheme;
  window.toggleTheme = function(){
    var before=document.documentElement.getAttribute('data-theme') === 'light' ? 'light' : 'dark';
    if(typeof previousToggle === 'function') previousToggle.apply(this, arguments);
    var after=localStorage.getItem('pm_theme') || document.documentElement.getAttribute('data-theme') || (before === 'light' ? 'dark' : 'light');
    localStorage.setItem('pm_theme', after === 'light' ? 'light' : 'dark');
    localStorage.setItem('theme', after === 'light' ? 'light' : 'dark');
    setTimeout(function(){ syncThemeHard(); syncLayout(); }, 0);
  };
  var previousApplyTheme = window.applySavedTheme;
  window.applySavedTheme = function(){
    if(typeof previousApplyTheme === 'function') previousApplyTheme.apply(this, arguments);
    setTimeout(syncThemeHard, 0);
  };

  document.addEventListener('click', function(e){
    var t=e.target;
    if(t && t.closest && t.closest('#pmMobileSidebarToggle')){
      e.preventDefault();
      e.stopPropagation();
      window.toggleSidebar();
      return;
    }
    if(t && t.closest && t.closest('#pmMobileSidebarBackdrop')){
      e.preventDefault();
      closeSidebarHard();
      return;
    }
    if(mq.matches && t && t.closest && t.closest('#sidebar .menu-item')){
      setTimeout(closeSidebarHard, 120);
    }
  }, true);

  window.addEventListener('resize', function(){ setTimeout(syncLayout, 80); }, {passive:true});
  window.addEventListener('orientationchange', function(){ setTimeout(syncLayout, 180); }, {passive:true});
  document.addEventListener('DOMContentLoaded', syncLayout, {once:true});
  setTimeout(syncLayout, 80);
  setTimeout(syncLayout, 650);
})();

/* =========================================================
   PROMANAGE ULTIMATE RUNTIME FIX - no loops
   Corrige definitivamente: drawer móvil sobre overlay + sidebar modo claro.
   ========================================================= */
(function(){
  'use strict';
  var mq = window.matchMedia('(max-width:1024px)');
  var placeholder = null;
  var lastLooseToggle = 0;
  function $(id){ return document.getElementById(id); }
  function dashboardVisible(){ var d=$('dashboardWrapper'); return !!d && getComputedStyle(d).display !== 'none'; }
  function currentTheme(){
    var t = localStorage.getItem('pm_theme') || localStorage.getItem('theme') || document.documentElement.getAttribute('data-theme') || document.body.getAttribute('data-theme') || 'dark';
  ...
  function moveSidebarIfNeeded(){
    var s=$('sidebar'), dash=$('dashboardWrapper');
    if(!s || !dash) return;
    if(mq.matches && dashboardVisible()){
      if(s.parentNode !== document.body){
        placeholder = placeholder || document.createComment('promanage-sidebar-return');
        if(!placeholder.parentNode) s.parentNode.insertBefore(placeholder, s);
        document.body.appendChild(s);
      }
    }else if(placeholder && placeholder.parentNode && s.parentNode === document.body){
      placeholder.parentNode.insertBefore(s, placeholder);
      placeholder.parentNode.removeChild(placeholder);
    }
  }
  function applySidebarLightInline(theme){
    var s=$('sidebar');
    if(!s) return;
    if(theme === 'light'){
      s.style.setProperty('background', '#f8fbff', 'important');
      s.style.setProperty('background-image', 'linear-gradient(180deg,#ffffff 0%,#eef6ff 100%)', 'important');
      s.style.setProperty('color', '#071426', 'important');
      s.style.setProperty('border-right', '1px solid #d1e3f8', 'important');
      s.style.setProperty('box-shadow', mq.matches ? '42px 0 120px rgba(15,50,90,.20)' : '18px 0 55px rgba(15,50,90,.12)', 'important');
    }else{
  ...
  function applyThemeUltimate(theme){
    theme = theme === 'light' ? 'light' : 'dark';
    document.documentElement.setAttribute('data-theme', theme);
    document.body.setAttribute('data-theme', theme);
    localStorage.setItem('pm_theme', theme);
    localStorage.setItem('theme', theme);
    document.body.classList.toggle('pm-light-mode', theme === 'light');
    document.body.classList.toggle('pm-dark-mode', theme !== 'light');
    var label=$('themeToggleText');
    if(label) label.textContent = theme === 'light' ? 'Modo Oscuro' : 'Modo Claro';
    var meta=document.querySelector('meta[name="theme-color"]');
    if(meta) meta.setAttribute('content', theme === 'light' ? '#f4f8ff' : '#020713');
    applySidebarLightInline(theme);
  }
  function syncUltimate(){
    ensureControls();
    var visible=dashboardVisible();
    document.body.classList.toggle('pm-dashboard-active', visible);
    document.body.classList.toggle('pm-mobile-layout', visible && mq.matches);
    moveSidebarIfNeeded();
    applyThemeUltimate(currentTheme());
    var s=$('sidebar'), bd=$('pmMobileSidebarBackdrop'), btn=$('pmMobileSidebarToggle');
    if(!visible || !mq.matches){
      document.body.classList.remove('pm-sidebar-open');
      if(s){ s.classList.remove('active','pm-sidebar-visible'); s.style.removeProperty('transform'); }
      if(bd) bd.classList.remove('active');
      if(btn) btn.setAttribute('aria-expanded','false');
    }
  }
  function openDrawer(){
    ...
    if(s){
      s.classList.add('active','pm-sidebar-visible');
      s.style.setProperty('z-index','2147483600','important');
      s.style.setProperty('pointer-events','auto','important');
      if(mq.matches) s.style.setProperty('transform','translate3d(0,0,0)','important');
      applySidebarLightInline(currentTheme());
    }
    if(bd){
      bd.classList.add('active');
      bd.style.setProperty('z-index','2147483000','important');
      bd.style.setProperty('pointer-events','none','important');
    }
    if(btn){ btn.setAttribute('aria-expanded','true'); btn.style.setProperty('z-index','2147483640','important'); }
  }
  function closeDrawer(){
    ...
    if(s){
      s.classList.remove('active','pm-sidebar-visible');
      if(mq.matches) s.style.setProperty('transform','translate3d(-108%,0,0)','important');
      else s.style.removeProperty('transform');
      s.style.setProperty('pointer-events','auto','important');
    }
    if(bd){ bd.classList.remove('active'); bd.style.setProperty('pointer-events','none','important'); }
    if(btn) btn.setAttribute('aria-expanded','false');
  }
  window.toggleSidebar = function(force){
    if(force === true) return openDrawer();
    if(force === false) return closeDrawer();
    var now=Date.now();
    if(now - lastLooseToggle < 140) return;
    lastLooseToggle = now;
    var s=$('sidebar');
    if(document.body.classList.contains('pm-sidebar-open') || (s && (s.classList.contains('active') || s.classList.contains('pm-sidebar-visible')))) return closeDrawer();
    return openDrawer();
  };
  window.openMobileSidebar = openDrawer;
  window.closeMobileSidebar = closeDrawer;
  window.applySavedTheme = function(){ applyThemeUltimate(currentTheme()); };
  window.toggleTheme = function(){ applyThemeUltimate(currentTheme() === 'light' ? 'dark' : 'light'); syncUltimate(); };
  ...
  document.addEventListener('click', function(e){
    ...
    if(mq.matches && document.body.classList.contains('pm-sidebar-open')){
      if(t && t.closest && t.closest('#sidebar')){
        if(t.closest('.menu-item')) setTimeout(closeDrawer,120);
        return;
      }
      if(!(t && t.closest && t.closest('#pmMobileSidebarToggle'))) closeDrawer();
    }
  }, false);

  window.addEventListener('resize', function(){ setTimeout(syncUltimate,70); }, {passive:true});
  window.addEventListener('orientationchange', function(){ setTimeout(syncUltimate,160); }, {passive:true});
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', syncUltimate, {once:true}); else syncUltimate();
  setTimeout(syncUltimate,120);
  setTimeout(syncUltimate,900);
})();


/* =========================================================
   PROMANAGE FINAL FIX - PDF EJECUTIVO AISLADO + FICHA 360 MÓVIL
   ========================================================= */
(function(){
  'use strict';
  function $(id){ return document.getElementById(id); }
  function callToast(msg){ try{ if(window.showToast) return window.showToast(msg,'success'); if(window.premiumToast) return window.premiumToast(msg); }catch(e){} }
  function executivePrintStyles(){
    return `@page{size:A4;margin:14mm;}*{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}html,body{margin:0;padding:0;background:#fff;color:#0f172a;font-family:Inter,Arial,sans-serif}body{font-size:12px;line-height:1.45}.pm-print-exec{width:100%;max-width:184mm;margin:0 auto;color:#0f172a}.pm-print-cover{min-height:250mm;display:flex;flex-direction:column;justify-content:center;border-bottom:3px solid #0f172a;page-break-after:always;break-after:page}.pm-print-cover>p:first-child{font-size:11px;text-transform:uppercase;letter-spacing:.16em;font-weight:800;color:#1d4ed8;margin:0 0 10px}.pm-print-cover h1{font-size:42px;line-height:1.05;letter-spacing:-1.4px;margin:0 0 12px;color:#071426}.pm-print-cover p{font-size:13px;color:#475569;margin:0 0 12px}.pm-print-section{page-break-inside:avoid;break-inside:avoid;margin:0 0 22px;padding:0}.pm-print-section h2{font-size:20px;margin:0 0 12px;color:#071426;border-bottom:1px solid #dbe7f5;padding-bottom:8px}.pm-print-section p,.pm-print-section li{font-size:12px;color:#334155}.pm-print-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:14px 0}.pm-print-kpi{border:1px solid #dbe7f5;border-radius:12px;padding:12px;background:#f8fbff;min-height:70px}.pm-print-kpi span{display:block;color:#64748b;font-size:9px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;margin-bottom:6px}.pm-print-kpi strong{display:block;font-size:18px;line-height:1.15;color:#071426;word-break:break-word}.pm-print-bar{height:9px;border-radius:999px;background:#e2e8f0;overflow:hidden;margin:8px 0 12px}.pm-print-bar div{height:100%;background:linear-gradient(90deg,#0f62fe,#38bdf8);border-radius:inherit}.pm-print-table{width:100%;border-collapse:collapse;font-size:10.5px;margin-top:10px}.pm-print-table th,.pm-print-table td{border:1px solid #dbe7f5;padding:7px;vertical-align:top;text-align:left;color:#0f172a}.pm-print-table th{background:#eef6ff;color:#334155;text-transform:uppercase;font-size:9px;letter-spacing:.05em}ul{margin:8px 0 0 18px;padding:0}li{margin-bottom:6px}`;
  }
  function getExecutiveHTML(){
    try{ if(typeof executiveReportHTML === 'function') return executiveReportHTML(); }catch(e){}
    if(typeof window.executiveReportHTML === 'function') return window.executiveReportHTML();
    var preview = $('ceoReportPreview');
    return '<div class="pm-print-exec"><section class="pm-print-section"><h2>Reporte Ejecutivo</h2>' + (preview ? preview.innerHTML : '<p>No hay datos disponibles.</p>') + '</section></div>';
  }
  function printExecutiveIframe(){
    var old = $('pmExecutivePrintFrame');
    if(old) old.remove();
    var frame = document.createElement('iframe');
    frame.id = 'pmExecutivePrintFrame';
    frame.setAttribute('aria-hidden','true');
    frame.style.cssText = 'position:fixed;right:0;bottom:0;width:0;height:0;border:0;opacity:0;pointer-events:none;z-index:-1;';
    document.body.appendChild(frame);
    var doc = frame.contentDocument || frame.contentWindow.document;
    doc.open();
    doc.write('<!doctype html><html><head><meta charset="utf-8"><title>Reporte Ejecutivo ProManage</title><style>'+executivePrintStyles()+'</style></head><body>'+getExecutiveHTML()+'</body></html>');
    doc.close();
    var removed = false;
    function cleanup(){ if(removed) return; removed = true; setTimeout(function(){ try{ frame.remove(); }catch(e){} }, 900); }
    setTimeout(function(){
      try{
        frame.contentWindow.focus();
        frame.contentWindow.print();
        ...
        console.error('Error imprimiendo PDF ejecutivo:', error);
        alert('No se pudo abrir la impresión del PDF ejecutivo. Intenta de nuevo.');
        cleanup();
      }
      setTimeout(cleanup, 4000);
    }, 260);
    try{ if(typeof window.addTimelineEvent === 'function') window.addTimelineEvent('CEO Report ejecutivo exportado a PDF.', 'report'); if(typeof window.renderTimeline === 'function') window.renderTimeline(); }catch(e){}
    callToast('PDF ejecutivo preparado.');
  }
  window.exportExecutivePDF = printExecutiveIframe;
  if(window.PMEnterprise) window.PMEnterprise.exportExecutivePDF = printExecutiveIframe;

  function projectIdFromElement(el){
    if(!el) return '';
    var direct = '';
    try{ direct = (el.getAttribute && el.getAttribute('data-project-id')) || (el.dataset && el.dataset.projectId) || ''; }catch(e){}
    if(direct) return direct;
    var onclick = '';
    try{ onclick = (el.getAttribute && el.getAttribute('onclick')) || ''; }catch(e){}
    var m = String(onclick).match(/(?:openProjectDetailsModal|openEditProjectModal|deleteProject|handleDragStart)\([^'\"]*['\"]([^'\"]+)['\"]/);
    if(m) return m[1];
    var inside = el.querySelector && el.querySelector('[onclick*="Project"], [onclick*="handleDragStart"], [data-project-id]');
    return inside ? projectIdFromElement(inside) : '';
  }
  function addButton(container, projectId, mode){
    if(!container || !projectId || container.querySelector('.pm360-btn')) return;
    var b = document.createElement('button');
    b.type = 'button';
    b.className = 'action-text-btn pm360-btn pm360-btn-visible';
    b.textContent = 'Ficha 360';
    b.setAttribute('data-project-id', projectId);
    b.onclick = function(e){ e.preventDefault(); e.stopPropagation(); if(typeof window.openProject360 === 'function') window.openProject360(projectId); else alert('Ficha 360 no disponible todavía.'); };
    if(mode === 'prepend') container.insertBefore(b, container.firstChild); else container.appendChild(b);
  }
  function inject360Everywhere(){
    document.querySelectorAll('#projectsListTableBody tr').forEach(function(row){
      var id = projectIdFromElement(row);
      var td = row.querySelector('td:last-child') || row.lastElementChild;
      if(id && td) addButton(td, id, 'prepend');
    });
    document.querySelectorAll('.project-card').forEach(function(card){
      var id = projectIdFromElement(card);
      if(id) addButton(card.querySelector('.project-info') || card, id, 'append');
    });
    var detail = $('projectDetailsModal');
    if(detail && detail.classList.contains('active')){
      var id = window.selectedDetailProjectId || detail.getAttribute('data-project-id') || '';
      if(id){
        var content = detail.querySelector('.modal-content') || detail;
        var bar = detail.querySelector('.pm360-detail-bar');
        if(!bar){
          bar = document.createElement('div');
          bar.className = 'pm360-detail-bar';
          var title = detail.querySelector('#detailProjTitle') || content.firstElementChild;
          if(title && title.parentNode) title.parentNode.insertBefore(bar, title.nextSibling); else content.insertBefore(bar, content.firstChild);
        }
        addButton(bar, id, 'append');
      }
    }
  }
  var oldRenderProjects = window.renderProjectsHub;
  if(typeof oldRenderProjects === 'function' && !oldRenderProjects.__pm360MobileFixed){
    var wrapped = function(){ var r = oldRenderProjects.apply(this, arguments); setTimeout(inject360Everywhere, 40); return r; };
    wrapped.__pm360MobileFixed = true;
    window.renderProjectsHub = wrapped;
  }
  var oldOpenDetails = window.openProjectDetailsModal;
  if(typeof oldOpenDetails === 'function' && !oldOpenDetails.__pm360MobileFixed){
    var wrappedDetails = function(id){ var r = oldOpenDetails.apply(this, arguments); try{ var m=$('projectDetailsModal'); if(m) m.setAttribute('data-project-id', id || ''); }catch(e){} setTimeout(inject360Everywhere, 60); return r; };
    wrappedDetails.__pm360MobileFixed = true;
    window.openProjectDetailsModal = wrappedDetails;
  }
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(inject360Everywhere, 180); }, {once:true});
  document.addEventListener('click', function(){ setTimeout(inject360Everywhere, 120); }, true);
  window.addEventListener('resize', function(){ setTimeout(inject360Everywhere, 160); }, {passive:true});
  setTimeout(inject360Everywhere, 600);
})();

/* =========================================================
   PROMANAGE EXECUTIVE PDF PRO V2 - Reporte estructurado premium
   ========================================================= */
(function(){
  'use strict';
  const VERSION = 'exec-pdf-pro-v2';
  const $ = id => document.getElementById(id);
  const esc = v => String(v ?? '').replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#039;','"':'&quot;'}[c]));
  const low = v => String(v ?? '').toLowerCase().trim();
  const fmtDate = d => {
    try { return new Intl.DateTimeFormat('es-GT',{dateStyle:'long', timeStyle:'short'}).format(d instanceof Date ? d : new Date(d)); }
    catch(e){ return new Date().toLocaleString('es-GT'); }
  };
  const money = v => new Intl.NumberFormat('es-GT',{style:'currency',currency:'GTQ',maximumFractionDigits:2}).format(Number(v || 0));
  const percent = v => Math.round(Math.max(0, Math.min(100, Number(v || 0)))) + '%';
  function readJSON(key, fallback){ try{ const raw = localStorage.getItem(key); return raw ? JSON.parse(raw) : fallback; }catch(e){ return fallback; } }
  function getLiveRows(key, name){
    try{ if(name && Array.isArray(window[name])) return window[name]; }catch(e){}
    try{ if(name && eval('typeof '+name) !== 'undefined' && Array.isArray(eval(name))) return eval(name); }catch(e){}
    return readJSON(key, []);
  }
  function currentUser(){ try{ if(typeof currentUser !== 'undefined' && currentUser) return currentUser; }catch(e){} return readJSON('pm_currentUser', null); }
  function isAdmin(){ const u = currentUser(); return !!u && (u.id === 'admin' || u.role === 'admin' || low(u.email) === 'admin@promanage.com'); }
  function selectedCompanyId(){ try{ if(typeof selectedCompanyId !== 'undefined' && selectedCompanyId) return selectedCompanyId; }catch(e){} return localStorage.getItem('pm_selectedCompanyId') || ''; }
  function scope(rows){
    rows = Array.isArray(rows) ? rows : [];
    const cid = selectedCompanyId();
    if(isAdmin()) return rows;
    return rows.filter(x => !cid || !x || !x.companyId || String(x.companyId) === String(cid));
  }
  function companyName(){
    const cid = selectedCompanyId();
    const comps = getLiveRows('pm_companies','companies');
    const found = comps.find(c => String(c.id) === String(cid));
    if(found && found.name) return found.name;
    return isAdmin() ? 'Vista Administrador' : 'Organización';
  }
  function projects(){ return scope(getLiveRows('pm_projects','projects')); }
  function finances(){ return scope(getLiveRows('pm_finances','finances')); }
  function clients(){ return scope(getLiveRows('pm_clients','clients')); }
  function inventory(){ return scope(getLiveRows('pm_inventory','inventory')); }
  function employees(){ return scope(getLiveRows('pm_employees','employees')); }
  function members(){ return scope(getLiveRows('pm_members','members')); }
  function activity(){ return scope(getLiveRows('pm_activityLogs','activityLogs').concat(readJSON('pm_enterpriseTimeline',[]))); }
  function isExpense(item){ const t = low(item?.type || item?.tipo || item?.kind || ''); return t.includes('egreso') || t.includes('gasto') || t.includes('expense') || t.includes('costo'); }
  function financeTotals(){
    return finances().reduce((acc,f)=>{ const amount = Math.abs(Number(f.amount || f.net || f.total || 0)); if(isExpense(f)) acc.egresos += amount; else acc.ingresos += amount; return acc; }, {ingresos:0, egresos:0, get balance(){ return this.ingresos - this.egresos; }});
  }
  function projectTitle(p){ return p?.title || p?.name || p?.nombre || 'Proyecto sin nombre'; }
  function projectStatus(p){ return p?.status || p?.estado || 'pendiente'; }
  function projectPriority(p){ return p?.priority || p?.prioridad || 'media'; }
  function projectProgress(p){
    const explicit = Number(p?.progress ?? p?.avance ?? p?.percent ?? NaN);
    if(!Number.isNaN(explicit)) return Math.max(0, Math.min(100, explicit));
    const st = low(projectStatus(p));
    if(st.includes('complet') || st.includes('final') || st.includes('pagado')) return 100;
    if(st.includes('proceso') || st.includes('ejec') || st.includes('activo')) return 58;
    if(st.includes('pausa')) return 32;
    return 15;
  }
  function projectRisk(p){
    try{ if(window.PMEnterprise && typeof window.PMEnterprise.calculateProjectRisk === 'function') return window.PMEnterprise.calculateProjectRisk(p); }catch(e){}
    const prog = projectProgress(p);
    const pr = low(projectPriority(p));
    let score = prog < 35 ? 70 : prog < 65 ? 45 : 22;
    if(pr.includes('alta')) score += 12;
    if(low(projectStatus(p)).includes('complet')) score = 12;
    const level = score >= 68 ? 'Alto' : score >= 42 ? 'Medio' : 'Bajo';
    return {score, level, progress:prog, recommendations:[level === 'Alto' ? 'Priorizar seguimiento, responsables y evidencias.' : 'Mantener seguimiento semanal y documentación actualizada.']};
  }
  function health(){
    const ps = projects();
    const totals = financeTotals();
    const risks = ps.map(projectRisk);
    const avgProgress = ps.length ? ps.reduce((a,p)=>a+projectProgress(p),0)/ps.length : 0;
    const completed = ps.filter(p => projectProgress(p) >= 100).length;
    const highRisk = risks.filter(r => r.level === 'Alto').length;
    const financeHealth = totals.ingresos > 0 ? Math.max(20, Math.min(100, 58 + (totals.balance / Math.max(totals.ingresos,1))*42)) : (totals.egresos > 0 ? 35 : 68);
    let score = Math.round((avgProgress * .34) + (financeHealth * .34) + ((ps.length ? completed/ps.length : 0.65)*100 * .16) + Math.min(100, activity().length*4) * .08 + 8 - highRisk*7);
    score = Math.max(0, Math.min(100, score));
    const state = score >= 82 ? 'Excelente' : score >= 67 ? 'Bueno' : score >= 48 ? 'Atención' : 'Crítico';
    return {score,state,avgProgress,completed,highRisk,financeHealth};
  }
  function topCategories(){
    const map = new Map();
    finances().forEach(f => { const k = String(f.category || f.concept || f.description || f.source || (isExpense(f)?'Egresos':'Ingresos')).trim() || 'Sin categoría'; map.set(k, (map.get(k)||0) + Math.abs(Number(f.amount || f.net || f.total || 0))); });
    return [...map.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5);
  }
  function countProjectImages(p){
    const medias = [].concat(Array.isArray(p?.media)?p.media:[], Array.isArray(p?.attachments)?p.attachments:[], Array.isArray(p?.files)?p.files:[]);
    return medias.filter(m => String(m?.type || m?.kind || m?.mime || '').includes('image') || String(m?.url || m?.data || '').startsWith('data:image')).length;
  }
  function recommendationList(h, totals, risks){
    const recs = [];
    if(h.highRisk > 0) recs.push(`Atender ${h.highRisk} proyecto(s) con riesgo alto antes de registrar nuevos compromisos.`);
    if(totals.balance < 0) recs.push('Revisar egresos y clasificar movimientos para recuperar balance positivo.');
    if(h.avgProgress < 55 && projects().length) recs.push('Definir próximos hitos por proyecto para elevar el avance promedio.');
    if(inventory().some(i => Number(i.stock || i.quantity || 0) <= 2)) recs.push('Actualizar inventario crítico y programar reposición de insumos con bajo stock.');
    if(projects().some(p => countProjectImages(p) === 0)) recs.push('Agregar evidencias visuales a proyectos sin documentación fotográfica.');
    risks.slice(0,3).forEach(x => { if(x.r?.recommendations?.[0]) recs.push(`${projectTitle(x.p)}: ${x.r.recommendations[0]}`); });
    if(!recs.length) recs.push('La operación está estable. Mantén respaldos periódicos, evidencias y seguimiento semanal.');
    return [...new Set(recs)].slice(0,8);
  }
  function styles(){
    return `@page{size:A4;margin:12mm 12mm 14mm;}*{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}html,body{margin:0;padding:0;background:#fff;color:#0a1324;font-family:Inter,Arial,Helvetica,sans-serif;font-size:11.5px;line-height:1.45}.report{width:100%;max-width:186mm;margin:0 auto}.page{page-break-after:always;break-after:page;position:relative;min-height:267mm;padding:0}.page:last-child{page-break-after:auto;break-after:auto}.cover{min-height:267mm;display:flex;flex-direction:column;justify-content:space-between;padding:12mm 0}.brand{display:flex;align-items:center;gap:12px}.logo{width:42px;height:42px;border-radius:13px;background:linear-gradient(135deg,#06111f,#0e66d8 54%,#7dd3fc);display:grid;place-items:center;color:#fff;font-weight:900;font-size:21px;box-shadow:0 14px 32px rgba(14,102,216,.28)}.brand h2{margin:0;font-size:18px;letter-spacing:-.04em}.brand span{display:block;color:#64748b;text-transform:uppercase;letter-spacing:.18em;font-size:8.8px;font-weight:800;margin-top:2px}.cover-main{border:1px solid #d7e3f1;border-radius:28px;padding:22mm 16mm;background:radial-gradient(circle at top right,rgba(14,102,216,.14),transparent 38%),linear-gradient(180deg,#f8fbff,#fff);box-shadow:0 28px 80px rgba(15,23,42,.08)}.eyebrow{margin:0 0 8px;text-transform:uppercase;letter-spacing:.2em;color:#0e66d8;font-size:9.5px;font-weight:900}.cover h1{font-size:48px;line-height:.98;letter-spacing:-2.1px;margin:0 0 10px;color:#071426}.subtitle{font-size:14px;color:#475569;max-width:132mm;margin:0}.meta-strip{display:grid;grid-template-columns:1.15fr .85fr .7fr;gap:10px;margin-top:16mm}.meta{border:1px solid #dbe7f5;border-radius:16px;padding:12px;background:#fff}.meta b{display:block;color:#071426;font-size:13px}.meta span{display:block;color:#64748b;font-size:9px;text-transform:uppercase;letter-spacing:.12em;font-weight:800;margin-bottom:4px}.cover-footer{display:flex;justify-content:space-between;color:#64748b;font-size:10px;border-top:1px solid #dbe7f5;padding-top:10px}.section-title{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin:0 0 12px;padding-bottom:9px;border-bottom:2px solid #0b1f3a}.section-title h2{font-size:22px;letter-spacing:-.7px;margin:0;color:#071426}.section-title p{margin:0;color:#64748b;font-size:10px;text-transform:uppercase;letter-spacing:.12em;font-weight:850}.grid{display:grid;gap:10px}.grid.cols-4{grid-template-columns:repeat(4,1fr)}.grid.cols-3{grid-template-columns:repeat(3,1fr)}.grid.cols-2{grid-template-columns:repeat(2,1fr)}.kpi{border:1px solid #dbe7f5;border-radius:16px;padding:12px;background:linear-gradient(180deg,#fff,#f8fbff);min-height:76px}.kpi span{display:block;color:#64748b;text-transform:uppercase;letter-spacing:.11em;font-size:8.6px;font-weight:900;margin-bottom:7px}.kpi strong{display:block;font-size:20px;line-height:1.05;letter-spacing:-.6px;color:#071426}.kpi small{display:block;color:#64748b;margin-top:5px;font-size:9.5px}.hero-score{display:grid;grid-template-columns:48mm 1fr;gap:14px;margin:14px 0}.score-card{border-radius:22px;padding:18px;background:linear-gradient(145deg,#071426,#0e66d8);color:#fff;min-height:54mm;display:flex;flex-direction:column;justify-content:space-between}.score-card span{text-transform:uppercase;letter-spacing:.14em;font-size:9px;font-weight:900;color:#b9d9ff}.score-card strong{font-size:52px;letter-spacing:-2px;line-height:.9}.score-card em{font-style:normal;font-weight:800;font-size:14px;color:#e0f2fe}.summary-box{border:1px solid #dbe7f5;border-radius:22px;padding:16px;background:#fff}.summary-box h3{margin:0 0 8px;font-size:16px}.summary-box p{margin:0 0 8px;color:#334155}.bar{height:10px;border-radius:999px;background:#e2e8f0;overflow:hidden}.bar i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#0e66d8,#38bdf8);width:var(--w,0%)}.finance-block{display:grid;grid-template-columns:1.1fr .9fr;gap:12px;margin-top:12px}.panel{border:1px solid #dbe7f5;border-radius:18px;background:#fff;padding:14px}.panel h3{font-size:15px;margin:0 0 10px;color:#071426}.bars{display:grid;gap:9px}.rowbar{display:grid;grid-template-columns:26mm 1fr 24mm;gap:8px;align-items:center;color:#334155}.rowbar .track{height:9px;border-radius:999px;background:#e2e8f0;overflow:hidden}.rowbar .fill{height:100%;border-radius:999px;background:#0e66d8}.rowbar.expense .fill{background:#ef4444}.rowbar.balance .fill{background:#10b981}table{width:100%;border-collapse:separate;border-spacing:0;margin-top:8px;font-size:10.2px}th{background:#eef6ff;color:#334155;text-transform:uppercase;letter-spacing:.08em;font-size:8.5px;font-weight:900;text-align:left;padding:8px;border-top:1px solid #dbe7f5;border-bottom:1px solid #dbe7f5}th:first-child{border-left:1px solid #dbe7f5;border-radius:10px 0 0 10px}th:last-child{border-right:1px solid #dbe7f5;border-radius:0 10px 10px 0}td{padding:8px;border-bottom:1px solid #edf3fa;color:#0f172a;vertical-align:top}tr:nth-child(even) td{background:#fbfdff}.pill{display:inline-flex;align-items:center;border-radius:999px;padding:3px 7px;font-size:8.5px;font-weight:900;text-transform:uppercase;letter-spacing:.06em}.risk-alto{background:#fee2e2;color:#991b1b}.risk-medio{background:#fef3c7;color:#92400e}.risk-bajo{background:#dcfce7;color:#166534}.checklist{display:grid;gap:8px;margin-top:10px}.action{display:grid;grid-template-columns:8mm 1fr;gap:8px;padding:10px;border:1px solid #dbe7f5;border-radius:14px;background:#fbfdff;break-inside:avoid}.num{width:24px;height:24px;border-radius:9px;background:#0e66d8;color:#fff;display:grid;place-items:center;font-size:11px;font-weight:900}.action p{margin:0;color:#334155}.footer{position:absolute;left:0;right:0;bottom:0;display:flex;justify-content:space-between;border-top:1px solid #dbe7f5;padding-top:7px;color:#64748b;font-size:9px}.two-col{columns:2;column-gap:18px}.activity{border-left:2px solid #0e66d8;padding-left:10px;margin-bottom:8px;color:#334155;break-inside:avoid}.activity b{color:#071426}.no-data{color:#64748b;font-style:italic;border:1px dashed #cbd5e1;border-radius:14px;padding:14px;text-align:center;background:#f8fafc}`;
  }
  function buildExecutiveHTML(){
    ...
    const totals = financeTotals();
    const h = health();
    const risks = ps.map(p => ({p, r:projectRisk(p)})).sort((a,b)=>(b.r.score||0)-(a.r.score||0));
    const maxFin = Math.max(totals.ingresos, totals.egresos, Math.abs(totals.balance), 1);
    const cats = topCategories();
    const lowStock = inv.filter(i => Number(i.stock || i.quantity || i.cantidad || 0) <= 2).slice(0,6);
    const recs = recommendationList(h, totals, risks);
    const avgRisk = risks.length ? Math.round(risks.reduce((a,x)=>a+(Number(x.r.score)||0),0)/risks.length) : 0;
    const preparedBy = currentUser()?.name || currentUser()?.email || 'Usuario ProManage';
    const org = companyName();
    const reportId = 'PM-' + new Date().toISOString().slice(0,10).replace(/-/g,'') + '-' + String(Date.now()).slice(-4);
    return `<div class="report" data-version="${VERSION}">
      <section class="page cover">
        <div class="brand"><div class="logo">P</div><div><h2>ProManage</h2><span>Gestión Integral de Proyectos</span></div></div>
        <div class="cover-main"><p class="eyebrow">Reporte ejecutivo profesional</p><h1>Estado general<br>de operación</h1><p class="subtitle">Documento estructurado con visión ejecutiva de proyectos, finanzas, riesgo, recursos y acciones recomendadas para toma de decisiones.</p><div class="meta-strip"><div class="meta"><span>Organización</span><b>${esc(org)}</b></div><div class="meta"><span>Preparado por</span><b>${esc(preparedBy)}</b></div><div class="meta"><span>ID Reporte</span><b>${esc(reportId)}</b></div></div></div>
        <div class="cover-footer"><span>Generado: ${esc(fmtDate(new Date()))}</span><span>Información local de ProManage</span></div>
      </section>
      <section class="page">
        <div class="section-title"><h2>Resumen ejecutivo</h2><p>Visión general</p></div>
        <div class="hero-score"><div class="score-card"><span>Health Score</span><strong>${h.score}</strong><em>${esc(h.state)}</em><div class="bar"><i style="--w:${percent(h.score)}"></i></div></div><div class="summary-box"><h3>Lectura rápida</h3><p>La operación registra <b>${ps.length}</b> proyecto(s), <b>${cls.length}</b> cliente(s) y un balance financiero de <b>${money(totals.balance)}</b>.</p><p>El avance promedio del portafolio es <b>${percent(h.avgProgress)}</b>, con <b>${h.highRisk}</b> proyecto(s) en riesgo alto y un índice de riesgo promedio de <b>${avgRisk}/100</b>.</p><p>Este reporte consolida únicamente la información disponible para el perfil/empresa actual; en perfil administrador incluye la vista global.</p></div></div>
        <div class="grid cols-4"><div class="kpi"><span>Ingresos</span><strong>${money(totals.ingresos)}</strong><small>Total registrado</small></div><div class="kpi"><span>Egresos</span><strong>${money(totals.egresos)}</strong><small>Costos y salidas</small></div><div class="kpi"><span>Balance</span><strong>${money(totals.balance)}</strong><small>Resultado neto</small></div><div class="kpi"><span>Actividad</span><strong>${acts.length}</strong><small>Últimos eventos</small></div></div>
        <div class="grid cols-4" style="margin-top:10px"><div class="kpi"><span>Proyectos</span><strong>${ps.length}</strong></div><div class="kpi"><span>Completados</span><strong>${h.completed}</strong></div><div class="kpi"><span>Inventario</span><strong>${inv.length}</strong></div><div class="kpi"><span>Equipo</span><strong>${emps.length + mems.length}</strong></div></div><div class="footer"><span>ProManage · Resumen ejecutivo</span><span>${esc(reportId)}</span></div>
      </section>
      <section class="page">
        <div class="section-title"><h2>Control financiero</h2><p>Ingresos · egresos · balance</p></div>
        <div class="finance-block"><div class="panel"><h3>Distribución financiera</h3><div class="bars"><div class="rowbar"><b>Ingresos</b><div class="track"><div class="fill" style="width:${percent((totals.ingresos/maxFin)*100)}"></div></div><span>${money(totals.ingresos)}</span></div><div class="rowbar expense"><b>Egresos</b><div class="track"><div class="fill" style="width:${percent((totals.egresos/maxFin)*100)}"></div></div><span>${money(totals.egresos)}</span></div><div class="rowbar balance"><b>Balance</b><div class="track"><div class="fill" style="width:${percent((Math.abs(totals.balance)/maxFin)*100)}"></div></div><span>${money(totals.balance)}</span></div></div></div><div class="panel"><h3>Mayor movimiento por categoría</h3>${cats.length ? `<table><thead><tr><th>Categoría</th><th>Monto</th></tr></thead><tbody>${cats.map(([k,v])=>`<tr><td>${esc(k)}</td><td>${money(v)}</td></tr>`).join('')}</tbody></table>` : `<div class="no-data">Aún no hay movimientos financieros registrados.</div>`}</div></div>
        <div class="panel" style="margin-top:12px"><h3>Movimientos recientes</h3><table><thead><tr><th>Tipo</th><th>Concepto</th><th>Origen</th><th>Monto</th></tr></thead><tbody>${fs.slice(-10).reverse().map(f=>`<tr><td>${isExpense(f)?'Egreso':'Ingreso'}</td><td>${esc(f.category || f.concept || f.description || 'Movimiento')}</td><td>${esc(f.source || f.project || '-')}</td><td>${money(Math.abs(Number(f.amount || f.net || f.total || 0)))}</td></tr>`).join('') || `<tr><td colspan="4">Sin movimientos registrados</td></tr>`}</tbody></table></div><div class="footer"><span>ProManage · Control financiero</span><span>${esc(reportId)}</span></div>
      </section>
      <section class="page">
        <div class="section-title"><h2>Portafolio de proyectos</h2><p>Avance y riesgo</p></div>
        <div class="grid cols-3"><div class="kpi"><span>Avance promedio</span><strong>${percent(h.avgProgress)}</strong></div><div class="kpi"><span>Riesgo alto</span><strong>${h.highRisk}</strong></div><div class="kpi"><span>Riesgo promedio</span><strong>${avgRisk}/100</strong></div></div>
        <table><thead><tr><th>Proyecto</th><th>Estado</th><th>Prioridad</th><th>Avance</th><th>Riesgo</th><th>Evidencias</th></tr></thead><tbody>${risks.slice(0,12).map(({p,r})=>`<tr><td><b>${esc(projectTitle(p))}</b><br><span style="color:#64748b">${esc((p.description || p.desc || '').slice(0,80))}</span></td><td>${esc(projectStatus(p))}</td><td>${esc(projectPriority(p))}</td><td><div class="bar"><i style="--w:${percent(r.progress ?? projectProgress(p))}"></i></div>${percent(r.progress ?? projectProgress(p))}</td><td><span class="pill risk-${low(r.level)}">${esc(r.level || 'Bajo')}</span></td><td>${countProjectImages(p)}</td></tr>`).join('') || `<tr><td colspan="6">Sin proyectos registrados</td></tr>`}</tbody></table>
        <div class="panel" style="margin-top:12px"><h3>Lectura del riesgo</h3><p style="margin:0;color:#334155">Los niveles de riesgo combinan avance, prioridad, posibles atrasos, presupuesto y documentación. Este bloque sirve para priorizar seguimiento operativo sin mezclar proyectos ajenos al perfil actual.</p></div><div class="footer"><span>ProManage · Portafolio</span><span>${esc(reportId)}</span></div>
      </section>
      <section class="page">
        <div class="section-title"><h2>Recursos y operación</h2><p>Inventario · equipo · actividad</p></div>
        <div class="grid cols-4"><div class="kpi"><span>Clientes</span><strong>${cls.length}</strong></div><div class="kpi"><span>Empleados</span><strong>${emps.length}</strong></div><div class="kpi"><span>Especialistas</span><strong>${mems.length}</strong></div><div class="kpi"><span>Stock bajo</span><strong>${lowStock.length}</strong></div></div>
        <div class="grid cols-2" style="margin-top:12px"><div class="panel"><h3>Insumos críticos</h3>${lowStock.length ? `<table><thead><tr><th>Insumo</th><th>Categoría</th><th>Stock</th></tr></thead><tbody>${lowStock.map(i=>`<tr><td>${esc(i.product || i.name || i.insumo || 'Insumo')}</td><td>${esc(i.category || '-')}</td><td>${esc(i.stock || i.quantity || 0)}</td></tr>`).join('')}</tbody></table>` : `<div class="no-data">No hay insumos críticos detectados.</div>`}</div><div class="panel"><h3>Actividad reciente</h3>${acts.length ? acts.map(a=>`<div class="activity"><b>${esc(a.title || a.type || 'Registro')}</b><br>${esc(a.message || a.description || a.action || '')}<br><span style="color:#64748b">${esc(a.date || a.createdAt || a.time || '')}</span></div>`).join('') : `<div class="no-data">Sin actividad reciente registrada.</div>`}</div></div><div class="footer"><span>ProManage · Recursos</span><span>${esc(reportId)}</span></div>
      </section>
      <section class="page">
        <div class="section-title"><h2>Plan de acción recomendado</h2><p>Prioridades ejecutivas</p></div>
        <div class="checklist">${recs.map((r,i)=>`<div class="action"><div class="num">${i+1}</div><p>${esc(r)}</p></div>`).join('')}</div>
        <div class="panel" style="margin-top:14px"><h3>Cierre ejecutivo</h3><p style="margin:0;color:#334155">Se recomienda usar este documento como punto de revisión semanal: actualizar evidencias, validar costos, revisar proyectos en riesgo y mantener respaldos recientes del sistema.</p></div><div class="footer"><span>ProManage · Plan de acción</span><span>${esc(reportId)}</span></div>
      </section>
    </div>`;
  }
  function printExecutivePDFPro(){
    const old = $('pmExecutivePrintFrame');
    if(old) old.remove();
    const frame = document.createElement('iframe');
    frame.id = 'pmExecutivePrintFrame';
    frame.setAttribute('aria-hidden','true');
    ...
    const doc = frame.contentDocument || frame.contentWindow.document;
    doc.open();
    doc.write('<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Reporte Ejecutivo ProManage</title><style>'+styles()+'</style></head><body>'+buildExecutiveHTML()+'</body></html>');
    doc.close();
    setTimeout(() => {
      try{
        frame.contentWindow.focus();
        frame.contentWindow.print();
        frame.contentWindow.onafterprint = () => setTimeout(()=>frame.remove(), 800);
      }catch(error){
        console.error('No se pudo imprimir el reporte ejecutivo profesional:', error);
        alert('No se pudo abrir la impresión del PDF ejecutivo. Intenta de nuevo.');
        frame.remove();
      }
    }, 320);
    try{ if(window.addTimelineEvent) window.addTimelineEvent('Reporte ejecutivo profesional exportado a PDF.', 'report'); if(window.renderTimeline) window.renderTimeline(); }catch(e){}
    try{ if(window.showToast) window.showToast('PDF ejecutivo profesional preparado.', 'success'); else if(window.premiumToast) window.premiumToast('PDF ejecutivo profesional preparado.'); }catch(e){}
  }
  window.exportExecutivePDF = printExecutivePDFPro;
  window.exportExecutivePDFPro = printExecutivePDFPro;
  if(window.PMEnterprise) window.PMEnterprise.exportExecutivePDF = printExecutivePDFPro;
})();
