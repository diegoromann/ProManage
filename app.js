...
/* ProManage stable: removed old layout polling and global capture listeners to prevent desktop freezes. */
const oldSwitch=window.switchSection;
...
/* =========================================================
   PROMANAGE STABLE FUNCTIONAL PATCH - no polling / no freeze
   Fixes: menú móvil, Ctrl+K, perfiles, PDF, modo claro y buscador.
   ========================================================= */
(function(){
  'use strict';
  ...
  function getUser(){ try{ if(typeof currentUser !== 'undefined' && currentUser) return currentUser; }catch(e){} return readJSON('pm_currentUser', null); }
  function userId(){ var u=getUser(); return String((u && u.id) || ''); }
  function isAdmin(){ var u=getUser(); return !!u && (u.id === 'admin' || u.role === 'admin' || String(u.email||'').toLowerCase() === 'admin@promanage.com'); }
  function getCompanyId(){ try{ if(typeof selectedCompanyId !== 'undefined' && selectedCompanyId) return String(selectedCompanyId); }catch(e){} return String(localStorage.getItem('pm_selectedCompanyId') || ''); }
  function companyOwnedByUser(companyId){
    if(!companyId) return false;
    var companies = readJSON('pm_companies', []);
    var uid = userId();
    return companies.some(function(c){ return String(c && c.id) === String(companyId) && String(c.createdBy || '') === uid; });
  }
  function visibleRecord(item){
    ...
    if(item.createdBy || item.userId || item.ownerUserId){
      return String(item.createdBy || item.userId || item.ownerUserId) === userId();
    }
    return companyOwnedByUser(cid);
  }
  function rows(key){ var v=readJSON(key, []); return Array.isArray(v) ? v : []; }
  function scopedRows(key){ return rows(key).filter(visibleRecord); }
  function projectArray(){ try{ if(typeof projects !== 'undefined' && Array.isArray(projects)) return projects; }catch(e){} return rows('pm_projects'); }
  function scopedProjects(){ return projectArray().filter(visibleRecord); }
  function tagNewest(storageKey, globalName, beforeLen){
    var uid=userId(), cid=getCompanyId(); if(!uid || !cid) return;
    try{
      var arr;
      try{ arr = (typeof window[globalName] !== 'undefined' && Array.isArray(window[globalName])) ? window[globalName] : null; }catch(e){ arr=null; }
      if(!arr) arr = rows(storageKey);
      if(!Array.isArray(arr)) return;
      arr.forEach(function(item, idx){
        if(idx >= beforeLen || (String(item.companyId||'') === cid && !item.createdBy)){
          item.companyId = item.companyId || cid;
          item.createdBy = item.createdBy || uid;
  ...
  function wrapSubmit(name, storageKey, globalName){
    var old = window[name];
    if(typeof old !== 'function' || old.__pmStableProfileWrapped) return;
    var wrapped = function(){
      var before = rows(storageKey).length;
      var result = old.apply(this, arguments);
      setTimeout(function(){ tagNewest(storageKey, globalName, before); refreshReports(); }, 30);
      return result;
    };
    wrapped.__pmStableProfileWrapped = true;
    window[name] = wrapped;
    try{ eval(name + ' = window["' + name + '"]'); }catch(e){}
  }
  function guardSelectedCompany(){
    if(isAdmin()) return;
    var cid = getCompanyId();
    if(!cid) return;
    if(!companyOwnedByUser(cid)){
      try{ selectedCompanyId = null; }catch(e){}
      localStorage.removeItem('pm_selectedCompanyId');
      var dash=$('dashboardWrapper'), picker=$('companyPickerWrapper'), auth=$('authWrapper');
  ...
  function dashboardVisible(){
    var dash=$('dashboardWrapper'); if(!dash) return false;
  ...
  function ensureMenu(){
    var oldWrong = document.getElementById('pmMobileMenuButton');
    if(oldWrong && oldWrong.id !== 'pmMobileSidebarToggle') oldWrong.remove();
  ...
  function openSidebar(){
    ...
    document.body.classList.add('pm-sidebar-open');
    var side=$('sidebar'); if(side) side.classList.add('active');
    var btn=$('pmMobileSidebarToggle'); if(btn) btn.setAttribute('aria-expanded','true');
    var bd=$('pmMobileSidebarBackdrop'); if(bd) bd.classList.add('active');
  }
  function closeSidebar(){
    document.body.classList.remove('pm-sidebar-open');
    var side=$('sidebar'); if(side) side.classList.remove('active');
    var btn=$('pmMobileSidebarToggle'); if(btn) btn.setAttribute('aria-expanded','false');
    var bd=$('pmMobileSidebarBackdrop'); if(bd) bd.classList.remove('active');
  }
  function toggleSidebar(){ document.body.classList.contains('pm-sidebar-open') ? closeSidebar() : openSidebar(); }
  window.openMobileSidebar = openSidebar;
  window.closeMobileSidebar = closeSidebar;
  window.toggleSidebar = function(force){ if(force === true) return openSidebar(); if(force === false) return closeSidebar(); return toggleSidebar(); };
  function refreshLayoutState(){
    ...
    if(!visible || !MOBILE.matches) closeSidebar();
    syncTheme();
  }
  document.addEventListener('click', function(ev){
    var t=ev.target;
    if(t && t.closest && t.closest('#pmMobileSidebarToggle')){ ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation(); toggleSidebar(); return; }
    if(t && t.closest && t.closest('#pmMobileSidebarBackdrop')){ ev.preventDefault(); closeSidebar(); return; }
    if(MOBILE.matches && t && t.closest && t.closest('#sidebar .menu-item')) setTimeout(closeSidebar, 120);
  }, true);

  /* Tema claro/oscuro sólido */
  function syncTheme(){
    var theme = localStorage.getItem('pm_theme') || localStorage.getItem('theme') || document.documentElement.getAttribute('data-theme') || 'dark';
    theme = theme === 'light' ? 'light' : 'dark';
    if(document.documentElement.getAttribute('data-theme') !== theme) document.documentElement.setAttribute('data-theme', theme);
    if(document.body && document.body.getAttribute('data-theme') !== theme) document.body.setAttribute('data-theme', theme);
    var label=$('themeToggleText'); if(label) label.textContent = theme === 'light' ? 'Modo Oscuro' : 'Modo Claro';
    var meta=document.querySelector('meta[name="theme-color"]'); if(meta) meta.setAttribute('content', theme === 'light' ? '#eef5fb' : '#020611');
  }
  window.toggleTheme = function(){
  ...
  function runAction(action){
    if(action === 'dash'){
      ...
      if(typeof switchSection === 'function') switchSection('dash');
      setTimeout(function(){
        var el=$('dashPomodoroDisplay');
        if(el){ el.scrollIntoView({behavior:'smooth', block:'center'}); el.style.filter='drop-shadow(0 0 18px rgba(29,140,255,.7))'; setTimeout(function(){ el.style.filter=''; }, 1200); }
      },160);
    }else if(action && typeof switchSection === 'function') switchSection(action);
    closeCommand();
  }
  function closeCommand(){ var p=$('pmStableCommand'); if(p) p.remove(); }
  function openStableCommand(){
    closeCommand();
    var palette=document.createElement('div');
    palette.id='pmStableCommand';
    palette.className='pm-command-stable pm-fixed-command';
    palette.style.cssText='position:fixed;inset:0;z-index:999999999;display:flex;align-items:flex-start;justify-content:center;padding-top:10vh;';
    var actions=[
      ['Panel General','Abrir Dashboard','dash'],
      ['Temporizador','Abrir Pomodoro','pomodoro'],
      ['Finanzas','Abrir Finanzas','finances'],
      ['Proyectos','Abrir Proyectos','projects'],
      ['Reportes','Abrir Reportes','reports'],
      ['Centro Ejecutivo','Abrir Centro Ejecutivo','intelligence']
    ];
    palette.innerHTML='<div class="pm-fixed-command-box" style="width:min(760px,92vw);overflow:hidden;"><input id="pmStableCommandInput" placeholder="Buscar acciones..." autocomplete="off" style="width:100%;padding:22px 24px;font-size:17px;outline:none;"><div id="pmStableCommandResults"></div></div>';
    document.body.appendChild(palette);
    var input=$('pmStableCommandInput'), results=$('pmStableCommandResults');
    function render(q){
      q=String(q||'').toLowerCase();
      results.innerHTML='';
      actions.filter(function(a){ return !q || (a[0]+' '+a[1]).toLowerCase().indexOf(q)>-1; }).forEach(function(a){
        var item=document.createElement('div'); item.className='pm-fixed-command-item'; item.innerHTML='<span>'+text(a[1])+'</span><small>'+text(a[0])+'</small>'; item.onclick=function(){ runAction(a[2]); }; results.appendChild(item);
      });
    }
    render('');
    input.addEventListener('input', function(){ render(input.value); });
    input.addEventListener('keydown', function(e){ if(e.key==='Escape') closeCommand(); if(e.key==='Enter'){ var first=results.querySelector('.pm-fixed-command-item'); if(first) first.click(); } });
    palette.addEventListener('click', function(e){ if(e.target === palette) closeCommand(); });
    setTimeout(function(){ input.focus(); }, 30);
  }
  window.openCommandPalette = openStableCommand;
  document.addEventListener('keydown', function(e){
    if((e.ctrlKey || e.metaKey) && String(e.key).toLowerCase() === 'k'){
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); openStableCommand();
    }
    if(e.key === 'Escape') closeCommand();
  }, true);

  /* Buscador desplegable con lupa */
  function patchSearch(){
    var shell=$('pageSearchShell'), input=$('globalSearchInput');
    if(!shell || !input) return;
    if(input.value) shell.classList.add('active','has-query');
  }
  document.addEventListener('click', function(ev){
    var shell=$('pageSearchShell'), input=$('globalSearchInput');
    if(!shell || !input) return;
    if(ev.target && ev.target.closest && ev.target.closest('#pageSearchShell .icon-search-btn')){
      ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation();
      shell.classList.add('active'); setTimeout(function(){ input.focus(); }, 20); return;
    }
    if(ev.target && ev.target.closest && ev.target.closest('#pageSearchShell')) return;
    if(!input.value){ shell.classList.remove('active','has-query','has-results'); }
  }, true);
  document.addEventListener('input', function(ev){
    if(ev.target && ev.target.id === 'globalSearchInput'){
      var shell=$('pageSearchShell'); if(shell){ shell.classList.add('active'); shell.classList.toggle('has-query', !!ev.target.value); }
    }
  }, true);

  /* Reportes/PDF solo del perfil salvo admin */
  function projectImages(project){
    var out=[], seen={}; var pid=String(project && project.id || '');
    function add(x){
      if(!x) return; var src=x.dataUrl || x.url || x.src || x.base64 || x.image || x.content || '';
      if(!String(src).match(/^data:image\//)) return;
      if(seen[src]) return; seen[src]=true;
      out.push({src:src,name:x.name || x.filename || x.title || 'Imagen',date:x.createdAt || x.date || '',size:x.size || 0});
    }
    function list(arr){ if(Array.isArray(arr)) arr.forEach(add); }
    try{ if(typeof window.getProjectImages === 'function') list(window.getProjectImages(pid)); }catch(e){}
    ['pm_projectImages','pm_project_images','pm_projectsImages','pm_gallery','pm_media','pm_attachments'].forEach(function(key){
      var data=readJSON(key,null); if(!data) return;
      if(Array.isArray(data)) list(data.filter(function(v){ return String(v && (v.projectId || v.project || v.parentId) || '') === pid; }));
      else list(data[pid]);
    });
    ['images','gallery','media','attachments','files','evidence','evidences'].forEach(function(k){ list(project && project[k]); });
    return out;
  }
  function refreshReports(){
    var select=$('reportProjectSelect'); if(!select) return;
    var current=select.value;
    var arr=scopedProjects();
    select.innerHTML='';
    if(!arr.length){ select.innerHTML='<option value="">No hay proyectos registrados para este perfil</option>'; return; }
    arr.forEach(function(p){ var opt=document.createElement('option'); opt.value=String(p.id||''); opt.dataset.projectId=String(p.id||''); opt.textContent=p.title || p.name || 'Proyecto sin nombre'; select.appendChild(opt); });
    if(Array.prototype.some.call(select.options,function(o){ return o.value === current; })) select.value=current;
  }
  window.pmRefreshReportProjects = refreshReports;
  window.renderReportProjects = refreshReports;
  window.populateReportProjects = refreshReports;
  function findScopedProject(id){
    id=String(id||'').trim(); if(!id) return null;
    var arr=scopedProjects();
    return arr.find(function(p){ return String(p.id||'')===id; }) || arr.find(function(p){ return String(p.title||p.name||'').trim().toLowerCase()===id.toLowerCase(); }) || null;
  }
  function reportHTML(project){
    var imgs=projectImages(project);
    var gallery=imgs.length ? imgs.map(function(img){ return '<figure><img src="'+img.src+'"><figcaption>'+text(img.name)+'</figcaption></figure>'; }).join('') : '<div class="empty">Este proyecto aún no tiene imágenes guardadas.</div>';
    return '<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Reporte '+text(project.title||project.name||'Proyecto')+'</title><style>@page{size:A4;margin:13mm}*{box-sizing:border-box}body{font-family:Inter,Arial,sans-serif;margin:0;color:#111827;background:#fff;font-size:13px;line-height:1.5;-webkit-print-color-adjust:exact;print-color-adjust:exact}.cover{padding:28px 0 24px;border-bottom:4px solid #0b63c7;margin-bottom:22px}.eyebrow{font-size:10px;text-transform:uppercase;letter-spacing:2px;color:#0b63c7;font-weight:900;margin:0 0 8px}h1{font-size:32px;line-height:1.05;margin:0 0 8px;letter-spacing:-1px}.sub{color:#4b5563}.meta{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin:20px 0}.card{border:1px solid #d7e1ee;background:#f8fbff;border-radius:12px;padding:12px}.card span{display:block;color:#64748b;font-size:10px;text-transform:uppercase;font-weight:900;letter-spacing:.08em}.card strong{display:block;margin-top:4px}.desc{background:#f8fbff;border-left:4px solid #0b63c7;border-radius:0 12px 12px 0;padding:16px;white-space:pre-wrap}.gallery-head{display:flex;justify-content:space-between;border-bottom:1px solid #e5e7eb;margin:26px 0 15px;padding-bottom:10px}.gallery{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.gallery figure{margin:0;border:1px solid #d7e1ee;border-radius:14px;padding:10px;break-inside:avoid}.gallery img{width:100%;height:215px;object-fit:cover;border-radius:9px;display:block}.gallery figcaption{margin-top:8px;font-size:11px;text-align:center;color:#475569}.empty{border:1px dashed #cbd5e1;border-radius:14px;background:#f8fafc;color:#64748b;padding:18px}.footer{margin-top:26px;border-top:1px solid #e5e7eb;padding-top:10px;color:#64748b;font-size:11px}</style></head><body><main><header class="cover"><p class="eyebrow">Reporte profesional de proyecto</p><h1>'+text(project.title||project.name||'Proyecto')+'</h1><p class="sub">Generado el '+new Date().toLocaleString('es-GT')+'</p></header><section class="meta"><div class="card"><span>Estado</span><strong>'+text(project.status||project.estado||'No definido')+'</strong></div><div class="card"><span>Prioridad</span><strong>'+text(project.priority||project.prioridad||'No definida')+'</strong></div><div class="card"><span>Avance</span><strong>'+text(project.progress||project.avance||'No registrado')+'</strong></div><div class="card"><span>Imágenes</span><strong>'+imgs.length+'</strong></div></section><section><h2>Información del proyecto</h2><div class="desc">'+text(project.description||project.desc||'Sin descripción registrada.')+'</div></section><section><div class="gallery-head"><h2>Galería del proyecto</h2><span>'+imgs.length+' imagen'+(imgs.length===1?'':'es')+'</span></div><div class="gallery">'+gallery+'</div></section><footer class="footer">Documento generado localmente por ProManage. En perfiles comunes solo incluye datos del usuario y empresa activa.</footer></main></body></html>';
  }
  function printDoc(html){
    var old=$('pmStablePrintFrame'); if(old) old.remove();
    var frame=document.createElement('iframe'); frame.id='pmStablePrintFrame'; frame.style.cssText='position:fixed;right:0;bottom:0;width:0;height:0;border:0;opacity:0;pointer-events:none'; document.body.appendChild(frame);
    var doc=frame.contentDocument || frame.contentWindow.document; doc.open(); doc.write(html); doc.close();
    setTimeout(function(){ var imgs=Array.prototype.slice.call(doc.images||[]); Promise.race([Promise.all(imgs.map(function(img){ return new Promise(function(r){ if(img.complete) r(); else{ img.onload=r; img.onerror=r; } }); })), new Promise(function(r){ setTimeout(r,2400); })]).then(function(){ frame.contentWindow.focus(); frame.contentWindow.print(); setTimeout(function(){ frame.remove(); }, 5000); }); },250);
  }
  window.generateSingleProjectPDF = function(){
    refreshReports();
    var select=$('reportProjectSelect'); var id=select && select.value;
    if(!id){ alert('Selecciona un proyecto'); return; }
    var p=findScopedProject(id);
    if(!p){ alert('Ese proyecto no pertenece a este perfil/empresa o ya no existe.'); refreshReports(); return; }
    printDoc(reportHTML(p));
    try{ if(typeof showToast === 'function') showToast('PDF preparado únicamente con el proyecto seleccionado.', 'success'); }catch(e){}
  };
  try{ window.filterCompany = function(items){ return Array.isArray(items) ? items.filter(visibleRecord) : []; }; }catch(e){}
  window.pmGetScopedRows = scopedRows;

  function patchNavigation(){
    var oldSwitch=window.switchSection;
    if(typeof oldSwitch === 'function' && !oldSwitch.__pmStableWrapped){
      var sw=function(section){ var r=oldSwitch.apply(this, arguments); setTimeout(function(){ refreshLayoutState(); if(section === 'reports') refreshReports(); patchSearch(); }, 80); if(MOBILE.matches) closeSidebar(); return r; };
      sw.__pmStableWrapped=true; window.switchSection=sw; try{ switchSection=sw; }catch(e){}
    }
    var oldSelect=window.selectCompany;
    if(typeof oldSelect === 'function' && !oldSelect.__pmStableWrapped){
      var sc=function(){ var r=oldSelect.apply(this, arguments); setTimeout(function(){ refreshLayoutState(); refreshReports(); tagAllVisible(); }, 100); return r; };
      sc.__pmStableWrapped=true; window.selectCompany=sc; try{ selectCompany=sc; }catch(e){}
    }
    var oldPicker=window.openCompanyPicker;
    if(typeof oldPicker === 'function' && !oldPicker.__pmStableWrapped){
      var pc=function(){ var r=oldPicker.apply(this, arguments); setTimeout(refreshLayoutState, 60); return r; };
      pc.__pmStableWrapped=true; window.openCompanyPicker=pc; try{ openCompanyPicker=pc; }catch(e){}
    }
    var oldLogout=window.logout;
    if(typeof oldLogout === 'function' && !oldLogout.__pmStableWrapped){
      var lo=function(){ var r=oldLogout.apply(this, arguments); setTimeout(function(){ closeSidebar(); refreshLayoutState(); }, 50); return r; };
      lo.__pmStableWrapped=true; window.logout=lo; try{ logout=lo; }catch(e){}
    }
  }
  function tagAllVisible(){ ['pm_projects:projects','pm_clients:clients','pm_finances:finances','pm_inventory:inventory','pm_members:members','pm_employees:employees'].forEach(function(pair){ var parts=pair.split(':'); tagNewest(parts[0], parts[1], rows(parts[0]).length); }); }
  function init(){
    ensureMenu(); syncTheme(); guardSelectedCompany(); patchNavigation(); patchSearch(); refreshReports(); refreshLayoutState();
    ...
    setTimeout(function(){ refreshLayoutState(); refreshReports(); }, 250);
    setTimeout(function(){ var s=$('pmSplash'), l=$('pmLoader'); [s,l].forEach(function(el){ if(el){ el.classList.add('hidden'); el.style.pointerEvents='none'; } }); }, 1600);
  }
...
/* =========================================================
   PROMANAGE FINAL STABILITY PATCH
   drawer móvil + tema claro sidebar + Project 360 estable
   ========================================================= */
(function(){
  'use strict';
  var MOBILE = window.matchMedia ? window.matchMedia('(max-width: 768px)') : {matches:false};
  function $(id){ return document.getElementById(id); }
  function safeText(v){ return String(v == null ? '' : v); }
  function esc(v){
    return safeText(v).replace(/[&<>"']/g,function(c){return ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[c];});
  }
  function arr(key){ try{ var v=JSON.parse(localStorage.getItem(key)||'[]'); return Array.isArray(v)?v:[]; }catch(e){ return []; } }
  function currentCompany(){ try{ return selectedCompanyId || localStorage.getItem('pm_selectedCompanyId') || ''; }catch(e){ return localStorage.getItem('pm_selectedCompanyId') || ''; } }
  function setDashboardState(){
    var dash=$('dashboardWrapper'), auth=$('authWrapper'), picker=$('companyPickerWrapper');
    var visible = !!(dash && dash.style.display !== 'none');
    document.body.classList.toggle('pm-dashboard-active', visible);
    if(visible){
      if(auth) auth.style.display='none';
      if(picker) picker.style.display='none';
      ['pmSplash','pmLoader'].forEach(function(id){ var el=$(id); if(el){ el.classList.add('hidden'); el.style.display='none'; el.style.pointerEvents='none'; } });
    } else {
      closeSidebar();
    }
  ...
  function ensureDrawerControls(){
    ...
    btn.className='pm-mobile-sidebar-toggle';
    btn.setAttribute('aria-label','Abrir menú');
    btn.setAttribute('aria-expanded', document.body.classList.contains('pm-sidebar-open') ? 'true':'false');
    btn.innerHTML='<span class="pm-bars" aria-hidden="true"></span><span class="pm-menu-label">Menú</span>';
  ...
  function syncDrawerVisibility(){
    ...
    var btn=$('pmMobileSidebarToggle');
    document.body.classList.toggle('pm-mobile-layout', visible && MOBILE.matches);
    if(btn) btn.style.display = (visible && MOBILE.matches) ? 'inline-flex' : 'none';
  ...
  function openSidebar(){
    ...
    var side=$('sidebar'), bd=$('pmMobileSidebarBackdrop'), btn=$('pmMobileSidebarToggle');
    document.body.classList.add('pm-sidebar-open');
    if(side){ side.classList.add('active'); side.style.pointerEvents='auto'; side.style.visibility='visible'; }
    if(bd){ bd.classList.add('active'); bd.style.pointerEvents='auto'; }
    if(btn) btn.setAttribute('aria-expanded','true');
  }
  function closeSidebar(){
    var side=$('sidebar'), bd=$('pmMobileSidebarBackdrop'), btn=$('pmMobileSidebarToggle');
    document.body.classList.remove('pm-sidebar-open');
    if(side) side.classList.remove('active');
    if(bd){ bd.classList.remove('active'); bd.style.pointerEvents='none'; }
    if(btn) btn.setAttribute('aria-expanded','false');
  }
  window.toggleSidebar = function(force){
    if(force === true) return openSidebar();
    if(force === false) return closeSidebar();
    return document.body.classList.contains('pm-sidebar-open') ? closeSidebar() : openSidebar();
  };
  window.openMobileSidebar = openSidebar;
  window.closeMobileSidebar = closeSidebar;
  ...
  function syncTheme(){
    var t='dark';
    try{ t=(localStorage.getItem('pm_theme') || localStorage.getItem('theme') || document.documentElement.getAttribute('data-theme') || 'dark'); }catch(e){}
    t = t === 'light' ? 'light' : 'dark';
    document.documentElement.setAttribute('data-theme',t);
    if(document.body) document.body.setAttribute('data-theme',t);
    var label=$('themeToggleText'); if(label) label.textContent = t === 'light' ? 'Modo Oscuro' : 'Modo Claro';
    var meta=document.querySelector('meta[name="theme-color"]'); if(meta) meta.setAttribute('content', t === 'light' ? '#f4f8fd' : '#020611');
  }
  var previousToggleTheme=window.toggleTheme;
  window.toggleTheme=function(){
    var now=document.documentElement.getAttribute('data-theme') === 'light' ? 'light':'dark';
    var next=now === 'light' ? 'dark':'light';
    try{ localStorage.setItem('pm_theme',next); localStorage.setItem('theme',next); }catch(e){}
    syncTheme();
    return false;
  };
  try{ if(typeof toggleTheme !== 'undefined') toggleTheme = window.toggleTheme; }catch(e){}

  function projectList(){ try{ if(Array.isArray(projects)) return projects; }catch(e){} return arr('pm_projects'); }
  function memberList(){ try{ if(Array.isArray(members)) return members; }catch(e){} return arr('pm_members'); }
  function financeList(){ try{ if(Array.isArray(finances)) return finances; }catch(e){} return arr('pm_finances'); }
  function inventoryList(){ try{ if(Array.isArray(inventory)) return inventory; }catch(e){} return arr('pm_inventory'); }
  function logsList(){ try{ if(Array.isArray(activityLogs)) return activityLogs; }catch(e){} return arr('pm_activityLogs'); }
  function findProject(id){ id=safeText(id); return projectList().find(function(p){ return safeText(p && p.id) === id; }); }
  function money(v){ var n=Number(v||0); return 'Q '+n.toLocaleString('es-GT',{minimumFractionDigits:2,maximumFractionDigits:2}); }
  function relatedString(row){ return safeText([row.projectId,row.project,row.projectTitle,row.category,row.concept,row.description,row.name,row.title,row.m,row.message].filter(Boolean).join(' ')).toLowerCase(); }
  function getImages(p){
    var pid=safeText(p && p.id), out=[], seen={};
    function add(x){
      if(!x) return;
      var src=x.dataUrl||x.url||x.src||x.base64||x.image||x.content||'';
      if(!/^data:image\//.test(safeText(src))) return;
      if(seen[src]) return; seen[src]=1;
      out.push({src:src,name:x.name||x.filename||x.title||'Evidencia'});
    }
    function list(a){ if(Array.isArray(a)) a.forEach(add); }
    try{ if(typeof window.getProjectImages === 'function') list(window.getProjectImages(pid)); }catch(e){}
    ['images','gallery','media','attachments','files','evidence','evidences'].forEach(function(k){ list(p && p[k]); });
    ['pm_projectImages','pm_project_images','pm_projectsImages','pm_gallery','pm_media','pm_attachments'].forEach(function(k){
      try{
        ...
        if(Array.isArray(d)) list(d.filter(function(x){ return safeText((x&&x.projectId)||(x&&x.project)||(x&&x.parentId)||'') === pid; }));
        else list(d[pid]);
      }catch(e){}
    });
    return out;
  }
  function relatedFinances(p){
    var pid=safeText(p.id), title=safeText(p.title||p.name).toLowerCase(), comp=currentCompany();
    return financeList().filter(function(f){
      if(comp && safeText(f.companyId||'') !== comp) return false;
      return safeText(f.projectId||'') === pid || (title && relatedString(f).indexOf(title)>-1);
    });
  }
  function relatedInventory(p){
    var pid=safeText(p.id), title=safeText(p.title||p.name).toLowerCase(), comp=currentCompany();
    return inventoryList().filter(function(i){
      if(comp && safeText(i.companyId||'') !== comp) return false;
      return safeText(i.projectId||'') === pid || (title && relatedString(i).indexOf(title)>-1);
    }).slice(0,10);
  }
  function relatedLogs(p){
    var pid=safeText(p.id).toLowerCase(), title=safeText(p.title||p.name).toLowerCase();
    return logsList().filter(function(l){ var s=relatedString(l); return s.indexOf(pid)>-1 || (title && s.indexOf(title)>-1); }).slice(0,12);
  }
  function ensure360(){
    var old=$('pmProject360Modal'); if(old){ old.classList.remove('active'); old.style.display='none'; old.style.pointerEvents='none'; }
    var modal=$('pm360StableModal');
    if(!modal){
      modal=document.createElement('div');
      modal.id='pm360StableModal';
      modal.innerHTML='<div class="pm360x-card" role="dialog" aria-modal="true" aria-labelledby="pm360StableTitle"><button type="button" class="pm360x-close" aria-label="Cerrar">×</button><div id="pm360StableContent"></div></div>';
      document.body.appendChild(modal);
      modal.addEventListener('click',function(ev){
        if(ev.target === modal || (ev.target.closest && ev.target.closest('.pm360x-close'))) closeProject360();
      });
    }
    return modal;
  }
  function closeProject360(){ var m=$('pm360StableModal'); if(m) m.classList.remove('pm360-stable-active'); }
  window.closeProject360=closeProject360;
  window.openProject360=function(id){
    var p=findProject(id); if(!p){ try{ showToast('Proyecto no encontrado','error'); }catch(e){ alert('Proyecto no encontrado'); } return; }
    var owner=memberList().find(function(m){ return safeText(m.id) === safeText(p.ownerId); });
    var imgs=getImages(p), fins=relatedFinances(p), inv=relatedInventory(p), logs=relatedLogs(p);
    var ingresos=0, egresos=0;
    fins.forEach(function(f){ var a=Number(f.amount||f.net||f.value||0); if(safeText(f.type).toLowerCase()==='egreso') egresos+=a; else ingresos+=a; });
    var gallery=imgs.length ? imgs.map(function(i){ return '<img src="'+i.src+'" alt="'+esc(i.name)+'" loading="lazy">'; }).join('') : '<div class="pm360x-empty">Este proyecto no tiene imágenes vinculadas todavía.</div>';
    var finRows=fins.length ? fins.map(function(f){ return '<div class="pm360x-row"><div><strong>'+esc(f.category||f.concept||f.name||'Movimiento')+'</strong><br><small>'+esc(f.date||f.createdAt||'Sin fecha')+'</small></div><strong>'+esc(f.type||'movimiento')+' · '+money(f.amount||f.net||f.value||0)+'</strong></div>'; }).join('') : '<div class="pm360x-empty">No hay movimientos financieros asociados solamente a este proyecto.</div>';
    var invRows=inv.length ? inv.map(function(i){ return '<div class="pm360x-row"><div><strong>'+esc(i.product||i.name||'Insumo')+'</strong><br><small>'+esc(i.category||'Inventario')+'</small></div><strong>'+esc(i.stock||i.qty||i.quantity||0)+' uds.</strong></div>'; }).join('') : '<div class="pm360x-empty">No hay inventario vinculado directamente a este proyecto.</div>';
    var logRows=logs.length ? logs.map(function(l){ return '<div class="pm360x-row"><div><strong>'+esc(l.m||l.message||l.title||'Actividad')+'</strong></div><small>'+esc(l.t||l.date||l.createdAt||'')+'</small></div>'; }).join('') : '<div class="pm360x-empty">No hay bitácora específica registrada para este proyecto.</div>';
    var modal=ensure360();
    var content=$('pm360StableContent');
    if(content){
      content.innerHTML='<section class="pm360x-hero"><div class="pm360x-kicker">Vista 360 del proyecto</div><h2 class="pm360x-title" id="pm360StableTitle">'+esc(p.title||p.name||'Proyecto')+'</h2><p class="pm360x-desc">'+esc(p.description||p.desc||'Sin descripción registrada.')+'</p><div class="pm360x-actions"><button class="pm360x-btn primary" type="button" onclick="try{ if(document.getElementById(\'reportProjectSelect\')) document.getElementById(\'reportProjectSelect\').value=\''+esc(p.id)+'\'; if(typeof generateSingleProjectPDF===\'function\') generateSingleProjectPDF(); else if(typeof generatePDFReport===\'function\') generatePDFReport(); }catch(e){}">Generar PDF</button><button class="pm360x-btn" type="button" onclick="try{openProjectDetailsModal(\''+esc(p.id)+'\')}catch(e){}">Abrir galería</button></div></section>'+
      '<section class="pm360x-grid"><div class="pm360x-kpi"><span>Estado</span><strong>'+esc(p.status||'No definido')+'</strong></div><div class="pm360x-kpi"><span>Prioridad</span><strong>'+esc(p.priority||'No definida')+'</strong></div><div class="pm360x-kpi"><span>Responsable</span><strong>'+esc((owner&&owner.name)||'Sin asignación')+'</strong></div><div class="pm360x-kpi"><span>Evidencias</span><strong>'+imgs.length+'</strong></div><div class="pm360x-kpi"><span>Ingresos</span><strong>'+money(ingresos)+'</strong></div><div class="pm360x-kpi"><span>Egresos</span><strong>'+money(egresos)+'</strong></div><div class="pm360x-kpi"><span>Balance</span><strong>'+money(ingresos-egresos)+'</strong></div><div class="pm360x-kpi"><span>ID</span><strong>'+esc(p.id)+'</strong></div></section>'+
      '<section class="pm360x-sections"><div class="pm360x-section"><h3>Galería vinculada</h3><div class="pm360x-gallery">'+gallery+'</div></div><div class="pm360x-section"><h3>Finanzas de este proyecto</h3><div class="pm360x-list">'+finRows+'</div></div><div class="pm360x-section"><h3>Inventario relacionado</h3><div class="pm360x-list">'+invRows+'</div></div><div class="pm360x-section"><h3>Actividad / bitácora</h3><div class="pm360x-list">'+logRows+'</div></div></section>';
    }
    modal.classList.add('pm360-stable-active');
  };
  function projectIdFromElement(el){
    if(!el) return '';
    var s=safeText(el.getAttribute('onclick')||'');
    var m=s.match(/open(?:Edit)?Project(?:Details)?Modal\('([^']+)'\)/) || s.match(/openProjectDetailsModal\('([^']+)'\)/) || s.match(/openEditProjectModal\('([^']+)'\)/);
    return m ? m[1] : '';
  }
  function inject360Buttons(){
    document.querySelectorAll('#projectsListTableBody tr').forEach(function(row){
      if(row.querySelector('.pm360-open-btn')) return;
      var source=row.querySelector('[onclick*="openEditProjectModal"]') || row.querySelector('[onclick*="openProjectDetailsModal"]');
      var id=projectIdFromElement(source); if(!id) return;
      var td=row.querySelector('td:last-child'); if(!td) return;
      var b=document.createElement('button'); b.type='button'; b.className='action-text-btn pm360-open-btn'; b.textContent='Vista 360'; b.addEventListener('click',function(ev){ev.preventDefault();ev.stopPropagation();window.openProject360(id);});
      td.insertBefore(b, td.firstChild);
    });
    document.querySelectorAll('.project-card').forEach(function(card){
      if(card.querySelector('.pm360-open-btn')) return;
      var source=card.querySelector('[onclick*="openProjectDetailsModal"]') || card.querySelector('[onclick*="openEditProjectModal"]');
      var id=projectIdFromElement(source); if(!id) return;
      var zone=card.querySelector('.project-info') || card;
      var b=document.createElement('button'); b.type='button'; b.className='action-text-btn pm360-open-btn'; b.textContent='Vista 360'; b.addEventListener('click',function(ev){ev.preventDefault();ev.stopPropagation();window.openProject360(id);});
      zone.appendChild(b);
  ...
  function wrapRenderProjects(){
    if(typeof window.renderProjectsHub === 'function' && !window.renderProjectsHub.__pmStable360){
      var old=window.renderProjectsHub;
      var wrapped=function(){ var r=old.apply(this,arguments); setTimeout(inject360Buttons,30); return r; };
      wrapped.__pmStable360=true;
      window.renderProjectsHub=wrapped;
      try{ renderProjectsHub=wrapped; }catch(e){}
    }
  }
  function initFinalPatch(){
    ...
    ensure360();
    inject360Buttons();
    setTimeout(function(){ setDashboardState(); syncTheme(); inject360Buttons(); },150);
    setTimeout(function(){ setDashboardState(); inject360Buttons(); },700);
  }
  document.addEventListener('click',function(ev){
    var t=ev.target;
  ...
  window.addEventListener('resize',function(){ setDashboardState(); },{passive:true});
  window.addEventListener('orientationchange',function(){ setTimeout(setDashboardState,160); },{passive:true});
  if(MOBILE.addEventListener) MOBILE.addEventListener('change',setDashboardState);
  var mo=new MutationObserver(function(){ setDashboardState(); });
  if(document.body) mo.observe(document.body,{attributes:true,attributeFilter:['class','data-theme']});
  var htmlMo=new MutationObserver(syncTheme);
  htmlMo.observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']});
  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded',initFinalPatch,{once:true}); else initFinalPatch();
})();
